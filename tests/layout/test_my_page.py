...
def test_page_size_margins(margin, top, right, bottom, left):
    (page,) = render_pages(
      ...
      padding: 1px; border: 2px solid; margin: %s }</style>"""
        % margin
    )
...
@assert_no_logs
@pytest.mark.parametrize(
    "html", ["<div>1</div>", "<div></div>", "<img src=pattern.png>"]
)
def test_page_breaks(html):
    ...
    assert [
        [child.position_y for child in page_child] for page_child in page_children
    ] == [[10, 40], [10, 40], [10]]
...
@assert_no_logs
@pytest.mark.parametrize(
    ("break_after", "margin_break", "margin_top"),
    [
        ("page", "auto", 5),
        ("auto", "auto", 0),
        ("page", "keep", 5),
...
@assert_no_logs
@pytest.mark.parametrize(
    ("direction", "page_break", "pages_number"),
    [
        ("ltr", "recto", 3),
        ("ltr", "verso", 2),
        ("rtl", "recto", 3),
...
@assert_no_logs
@pytest.mark.parametrize(
    ("direction", "page_break", "first_page"),
    [
        ("ltr", "recto", "right"),
        ("ltr", "verso", "left"),
        ("rtl", "recto", "left"),
...
@pytest.mark.parametrize(
    ...
    [
     ...
     """
            ...
            [200, 10],
        ),  # 'width' and other have no effect without 'content'
        (
...
def test_page_style(css, widths):
    expected_at_keywords = [
        at_keyword
        for at_keyword in ["@top-left", "@top-center", "@top-right"]
...
def test_margin_boxes_element():
    ...
    footer1_text = "".join(
        getattr(node, "text", "") for node in pages[0].children[1].descendants()
    )
    assert footer1_text == "0 of 3 (1)"

    footer2_text = "".join(
        getattr(node, "text", "") for node in pages[1].children[1].descendants()
    )
    assert footer2_text == "0 of 3 (2)"

    footer3_text = "".join(
        getattr(node, "text", "") for node in pages[2].children[1].descendants()
    )
...
def test_running_elements_display():
    ...
    assert "".join(getattr(node, "text", "") for node in left.descendants()) == "inline"
    assert (
        "".join(getattr(node, "text", "") for node in center.descendants()) == "block"
    )
    assert "".join(getattr(node, "text", "") for node in right.descendants()) == "table"
...
@assert_no_logs
def test_page_size_fit_page_multidiv_1():
    pages = render_pages(
      """
      <style>
      @page { size: fit; margin-left: 11px; margin-top: 22px; margin-right: 33px; margin-bottom: 44px}
...
@assert_no_logs
def test_page_size_fit_page_multidiv_2():
    pages = render_pages(
      """
      <style>
      @page { size: fit; margin-left: 11px; margin-top: 22px; margin-right: 33px; margin-bottom: 44px}
      * {margin: 0; padding: 0}
      section {width: 100px; height: 200px; margin: 10px; padding: 7px; border: 3px solid;box-sizing: content-box;}
      div.one {width: 555px; height: 200px;position: absolute; top:333px; left: 0; margin: 5px}
      div.two {width: 50px; height: 666px;position: absolute; top:333px; left: 55px; margin: 7px}
      </style>
      <body>
      <section></section>
      <div class="one"></div>
    ...
    assert len(pages) == 1
    page = pages[0]
    assert int(page.width) == 11 + 5 + 555 + 5 + 33 + 1
    assert int(page.height) == 22 + 7 + 333 + 666 + 7 + 44 + 1

@assert_no_logs
def test_page_size_fit_one_cant_disabled_more_than_one_page():
